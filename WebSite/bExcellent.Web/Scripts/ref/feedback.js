...
var feedback = {
    ...
    loaddetailview: function () {
        $('#contents').load('/client/detailedview_self_feedback.aspx', function (response, status, xhr) {
            // Common.animate('wcsi-inner-content', function () {
            var moduleIds = new Array();
            var poeid = 2;
            //details_feedback.loadDetailModuleIds({ url: serviceUrl + 'feedback.svc/GetPracticeArea', data: { 'poeid': poeid }, moduleIds: moduleIds });
            details_feedback.loadDetailViewPAQuestions({ url: serviceUrl + 'feedback.svc/GetQuestions', data: { 'moduleId': 2 }, moduleorder: 2 });
...
var details_feedback = {
    loadDetailViewPAQuestions: function (option) {
        ...
        Common.ajax({
            ...
            success: function (response) {
                //                debugger;
                var questionNo = parseInt(($('#question1').val() == '' ? '0' : $('#question1').val()));
                ...
                $('#next').live('click', function () {
                    ...
                    if (questionNo < 5) {
                        ...
                        $('#Qno1').before(response[questionNo].QuetionText);
                        if (questionNo == 1)
                            $('.Qno').append('<p>' + (questionNo + 1) + '</p>');
                        else
                            $('.Qno').append('<p>' + (questionNo + 1) + '</p>');
    ...
    //    loadDetailModuleIds: function (option) {
    //        Common.ajax({
    //            url: option.url,
    //            data: (option.data) ? option.data : null,
    //            /*data: 'provide your data in json format',*/
    ...
    //                for (var i = 0; i < response.length; i++) {
    //                    option.moduleIds[response[i].ModuleOrderNumber] = response[i].ModuleId;
    //                    $('#li' + (i + 1)).val(response[i].ModuleId);
    //                }
    //                $('#currentmoduleOrder').val('2');
    //                var curmod = parseInt($('#currentmoduleOrder').val());
    //                var nextmod = curmod + 1;
    //                alert(nextmod);
    //                details_feedback.loadDetailViewPAQuestions({ url: serviceUrl + 'feedback.svc/GetQuestions', data: { 'moduleId': $('#module' + nextmod).val() }, moduleorder: nextmod });
    //                details_feedback.loaddetailslider();
    //                $('#currentmoduleOrder').val(nextmod);
    //                $('#li' + curmod).removeClass('act');
    //                $('#li' + nextmod).addClass('act');
