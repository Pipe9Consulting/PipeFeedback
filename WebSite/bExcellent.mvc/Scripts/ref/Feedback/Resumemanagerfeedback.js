var manager_feedback = {
    ...
    //load last saved questions in detail view
    loadDetailViewLastSavedQuestions: function (option) {
        Common.ajax({
            ...
            success: function (response) {
                ...
                manager_feedback.loadCompletedQuestionCount();
                $('#currentQuestionid').val(response[parseInt($('#lastSavedQuestionOrder').val()) - 1].QuestionId);
                var SideBarContent = response[parseInt($('#lastSavedQuestionOrder').val()) - 1].SideBarTitle;
    ...
    loadModuleIds: function (option) {
        ...
        Common.ajax({
            ...
            success: function (response) {
                ...
                manager_feedback.loadDetailViewLastSavedQuestions({ url: '../../Feedback/GetQuestions', data: { 'moduleId': $('#lastSavedModuleId').val() }, moduleorder: $('#lastSavedModuleOrder').val() });
                manager_feedback.loadPoeContent({ url: '../../Feedback/GetPoeIntoContent' }, '#poeintro');
                manager_feedback.loadPoeContent({ url: '../../Feedback/GetModuleIntoContent', data: { 'moduleId': $('#lastSavedModuleOrder').val()} }, '#moduleintro');
                manager_feedback.loadPoeContent({ url: '../../Feedback/GetModuleKeyAction', data: { 'moduleId': $('#lastSavedModuleOrder').val()} }, '#modulekeyaction');

                for (var j = parseInt($('#lastSavedModuleOrder').val()) - 1; j > 0; j--) {

                    manager_feedback.loadCompletedPracticeArea($('#li' + j).data().value, j);
                }
    ...
    loadTeammembers: function (option) {
        Common.ajax({
            ...
            success: function (response) {
                ...
                //var html = '<ul>' + template({ 'team': _team }) + '</ul>';
