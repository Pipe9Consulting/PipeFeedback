$(function () {
    ...
    $('.slidebtn .nxt').click(function () {
        ...
        if ((currentFrameIndex + 1) == parseInt($('#framecount').val() / 10)) {
            $('.nxt').hide();
        }
    });
    ...
    $('.close,.reply').click(function () {
        $('.poepopup,.poepopupbg,.cmntpopup,.wallgoal,#videopopupcontainer').fadeOut("slow");
    });
    ...
    $('#ShareGroupAndUser').live('click', function () {
        ...
        if (groupid) {
            ...
            $('.yammerpopup,.yammerpopupbg').hide();
    ...
    $('#syncyammer').click(function () {
        ...
        if ($('.commentsel').length == 0) {
            $('#masterMsgCont').text("Please select messages from Wall to Sync with Yammer");
            $('#overallCont').show();
    ...
    $('.videosimg').live('click', function () {
        ...
        $("#vidprev").html("<script src='../../Scripts/ref/FlowPlayer/flowplayer-3.2.6.min.js' type='text/javascript'></script><div class='player' style=' width:540px;height:200px;' href='" + $(this).data('href') + "'></div>");
        ...
        $('#videopopupcontainer,.poepopupbg').fadeIn("slow");
    });
