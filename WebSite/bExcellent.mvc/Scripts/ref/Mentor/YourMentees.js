$(document).ready(function () {
    var poe = $('#selectedPoeValueYourMentees').val();
    ...
    yourMentees.loadPoeName({ url: '../../Mentor/GetPoeName' });
    yourMentees.loadModuleIds({ url: '../../Mentor/GetPracticeArea' });
    yourMentees.getuserRoles({ url: '../../Mentor/GetUserRoles' });
    yourMentees.getYourMentees();
   
    // yourMentees.getuserPoeByType();
    ...
    $('.poemodule').live('click', function () {
        ...
        //var a = yourMentees.loadQuickViewPAQuestions.arguments.img;
        var ctrlPoeModule = $(this);
        var viewVal = ctrlPoeModule.attr('data-value');
        yourMentees.HighlightTile(this);
        if (viewVal != 0) {
        ...
        //$('#' + viewVal).siblings().css('display', 'none')
            ...
            //yourMentees.setSelectedPoe(ctrlPoeModule.val());
           // var i = yourMentees.loadMdlIds({ url: '../../Mentor/GetPracticeArea' });

            $('#selectedPoeValueYourMentees').val(ctrlPoeModule.val());
        }
    ...
    $('.yourMenteesViewMessage').live('click', function () {
        ...
        var fromId = $(this).parents().eq(1).data('mgrid');
        $('#selectedidyourmentee').val(fromId);
        if (fromId != 0) {
            yourMentees.getConnectHistory(fromId)
        }
    ...
    $('#wallactcontent #sndConnect').live('click', function () {
        ...
        var selectedids = $('#selectedidyourmentee').val();
        if (selectedids != "" && sendmessagetext != "") {
            SendMessagesYourMentees(selectedids);
        } else {
            //$(".popupbg,.popup").show();
...
var yourMentees = {
    ...
    loadModuleIds: function (option) {
        ...
        Common.ajaxsync({
            ...
            success: function (response) {
                yourMentees.loadQuickViewPAQuestions({ url: '../../Mentor/GetQuestions', response: response });
    ...
    getuserRoles: function () {
        ...
        Common.ajaxsync({
            ...
            success: function (response) {
                ...
                yourMentees.getuserPoeByType(type);
                var typevalue = $('#yourMenteesType').val();
    ...
    loadPoeName: function () {
        ...
        var poeid = $('#selectedPoeValueYourMentees').val();
    ...
    loadMdlIds: function (option) {
        ...
        Common.ajaxsync({
            url: option.url,
            success: function (response) {
                yourMentees.loadImageCount({ url: '../../Mentor/GetQuestions', response: response });
    ...
    getuserPoeByType: function (type) {
        Common.ajaxsync({
            ...
            success: function (response) {
                ...
                var mentorPageSelectedVal = $('#selectedPoeValueYourMentees').val();
    ...
    getYourMentees: function () {
        Common.ajaxsync({
            url: '/Mentor/getYourMentees',
            success: function (response) {
                var html = '';
                if (response != null) {
                ...
                for (var i = 0; i < numberOfSlides; i++) {
                    ...
                    for (var j = (8 * i) ; j < responseCount && j < ((i + 1) * 8) ; j++) {
                        ...
                        if (response[i].user != null && response[i].user[0] != null) {
                            ...
                            html += "<li class='netmgr' data-mgrid='" + response[i].user[j].UserId + "' data-email='" + response[i].user[j].EmailAddress + "'>" +
                    ...
                    if (response.length < 8) {
                        for (var p = 0; p < (7 - response.length) ; p++) {
                ...
                $('#yourmenteeslist').html(html);

                $('#pagingNetwork').html((((responseCount / licount) > 1) ? yourMentees.getPaginationhtml((responseCount / licount), 'coltwo') : ""));
                yourMentees.getpaginationcontrols();
                if (count1 != 0) {
...
function SendMessagesYourMentees(selectedIds) {
    ...
    Common.ajax({
        ...
        success: function (response) {
            yourMentees.getConnectHistory($('#selectedidyourmentee').val());
