    ...
    loadchartdata: function (chart, element) {
        ...
        for (var j = 0; j < x; j++) {
            ...
            if (chart.values[j].score != 0) {
                ...
                $("#" + element + 'pro' + (j + 1) + " p").html(chart.values[j].score + "%");
            } else {
    ...
    LoadPoeDetailContent: function () {
        Common.ajaxsync({
            ...
            success: function (response) {
                ...
                var html = "<tr> <th>Practice Area</th> <th>Priority</th> <th>Points</th> </tr><tr>    <td colspan='3'><table id='bigpicPoeContent1Scroll'><tbody>";
    ...
    if (clickDashboardMode != "") {
        switch (parseInt(clickDashboardMode)) {
            ...
            case 2:
                ...
                $('#bigslide').hide();
                $('#nextinline').addClass('selected').siblings().removeClass('selected');
                $('#bigslide .prev,#bigslide .nxt').hide();
                break;
            case 3:
                $('#tenure').show();
